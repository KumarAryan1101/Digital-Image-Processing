...
class BitPlaneSlicingRGBApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RGB Bit-Plane Slicing")
    ...
    def upload_image(self):
        ...
        if file_path:
            try:
                ...
                self.rgb_image_np = np.array(self.original_image)
    ...
    def update_plane(self, k):
        ...
        k = int(k)
        # Algorithm: plane(k) = ((p >> k) & 1) * 255 for each channel
        plane_np = ((self.rgb_image_np >> k) & 1) * 255
        plane_np = plane_np.astype(np.uint8)
        
        self.current_plane_image = Image.fromarray(plane_np, "RGB")
    ...
    def generate_all_planes(self):
        ...
        h, w, c = self.rgb_image_np.shape
        # Create a 2x4 grid for 8 planes
        sheet_h = h * 2
        sheet_w = w * 4
        
        sheet = np.zeros((sheet_h, sheet_w, c), dtype=np.uint8)
    ...
    def view_all_planes(self):
        ...
        top.title("All RGB Bit Planes (7 to 0)")
    ...
    def save_current_plane(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"bit_plane_rgb_{self.plane_slider.get()}.png",
    ...
    def save_output_sheet(self):
        ...
        if sheet_img:
            file_path = filedialog.asksaveasfilename(
                defaultextension=".png",
                initialfile="bit_plane_slicing_RGB_output.png",
                filetypes=[("PNG files", "*.png"), ("JPEG files", "*.jpg"), ("All files", "*.*")]
            )
            if file_path:
                try:
...
if __name__ == "__main__":
    root = tk.Tk()
    app = BitPlaneSlicingRGBApp(root)
    root.mainloop()
